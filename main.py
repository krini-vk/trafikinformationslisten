    ...
    try:
        with open('log.txt', 'r') as log_file:
            ...
            if lines:
                ...
                return datetime.datetime.fromisoformat(last_line.split('---')[0].strip()).date()
            else:
                return None
...
def format_data(df: pd.DataFrame):
    ...
    df['road_info'] = df['oov2roaduserdescription'].apply(lambda x: x.split('\n')[0].strip() if isinstance(x, str) else None)
    ...
    df.rename(columns={'oov2roadinfo': 'title', 'contractorcontactperson': 'contractor_contact_person', 'contractormobile':'contractor_phone', 'ownermailaddress': 'contractor_email', 'name': 'contractor_company', 'oov2roadmarkstart': 'starttime', 'oov2roadmarkend':'endtime' }, inplace=True)
    ...
    df['duration'] = df.apply(
        lambda row: \
            ...
            if row['startdate'] == row['enddate'] and row['starttime'] is not None and row['endtime'] is not None \
...
def main():
    ...
    if last_log_date is not None:
        new_roadwork = df[df['startdate'] > str(last_log_date)].to_dict(orient="records")
        old_roadwork = df[df['startdate'] <= str(last_log_date)].to_dict(orient="records")
    else:
    ...
    doc_template.save(f'trafik informations listen {today_date}.docx')
